...
def back_substitution(U: List[List[float]], c: List[float]) -> Union[List[float], Tuple[List[List[float]], List[int]], str]:
...
def gaussian_eliminate(A: List[List[float]], b: List[float], verbose: bool = True) -> Tuple[List[List[float]], Any, int]:
    matrix_A = A
    vector_b = b
    """
    ...
    solution_x = back_substitution(upper_matrix_U, constant_vector_c)
