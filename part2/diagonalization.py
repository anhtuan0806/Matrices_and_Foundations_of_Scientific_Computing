...
def run_diagonalization_test(case_name: str, matrix_data: List[List[float]]) -> None:
    ...
    print("=" * 80)
    print("(*) Lưu ý: Thuật toán QR Iteration đưa ma trận về dạng Schur (tam giác trên).")
    print("    Việc trích xuất đường chéo chỉ tạo ra ma trận chéo chính xác D khi A đối xứng.")
    print("    Đối với ma trận không đối xứng, ||A - PDP^-1|| có thể sẽ sai lệch.")
    ...
    try:
        ...
        try:
            ...
            # Sắp xếp lại eigenvalues của Numpy để dễ so sánh (có thể khác thứ tự với thuật toán tự viết)
            eigenvalues_sorted = np.sort(eigenvalues)
            custom_eigenvalues = np.sort([matrix_D[i][i] for i in range(len(matrix_D))])
            
            diff_eigenvalues = np.linalg.norm(eigenvalues_sorted - custom_eigenvalues)
            print(f"\n=> KIỂM CHỨNG VỚI NUMPY:")
            print(f"   Sai lệch giá trị riêng (Eigenvalues) so với NumPy: {diff_eigenvalues:.5e}")
